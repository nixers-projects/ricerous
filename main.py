...
class InfoScreen(BoxLayout):
    ...
    def __init__(self, value="", **kwargs):
        ...
        txts = TextInput(text=inf, background_color=(0.15, 0.15, 0.15, 1),
                         foreground_color=(1, 1, 1, 1), multiline=True,
                         readonly=True, size_hint=(1.0, None))
        ...
        comments = TextInput(multiline=True, size_hint=(1.0, None),
    ...
    def on_text(self, instance, newText):
        ...
        for char in whiteSpace:
            defaultWOSpace = DEFAULT_COMMENT.replace(char, "")
            newTextWOSpace = newText.replace(char, "")
        if newTextWOSpace in defaultWOSpace or newTextWOSpace == "":
            if self.value in state.comments.keys():
...
class ButtonBar(BoxLayout):
    ...
    def save(self, path, filename):
        try:
            if self.outputing is True:
                selected = ""
                for choice in self.allChoices:
                    if choice.state == 'down':
                        selected = choice.text
                        break
                out.output(selected, state, jfile,
    ...
    def showUpdate(self):
        try:
            ...
            self._popup = Popup(title='Update', content=content,
                                size_hint=(0.6, 0.6))
            self._popup.open()
        except Exception:
    ...
    def showOutput(self):
        ...
        self._popup = Popup(title="Output", content=content,
                            size_hint=(0.9, 0.9))
